...
def _agent_node(state: AgentState):
    ...
    llm = _make_llm(cfg).bind_tools(merged_tools_for("chronos", TOOLS))
...
def _chronos_tools_node(state: AgentState):
    tools = merged_tools_for("chronos", TOOLS)
...
def build_chronos_agent():
    ...
    g.add_node("tools", _chronos_tools_node)
